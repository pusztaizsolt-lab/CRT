# CRT Ajánlatsegéd — Enterprise Architektúra Whitepaper v1.8
**Civil Rendszertechnika Kft. | Bizalmas belső dokumentum**
*Kiadás: 2026-06-20 | Szerző: Pusztai Zsolt + Claude Sonnet 4.6*
...
### Fázis 7 — Böngésző-komponens + Unified Attention Layer (2-3 hét)
- [ ] `scrapers/run_all.py` integrálása ARQ háttér-feladatba (ütemezett scraping)
- [ ] Playwright persistent browser profil — GDPR consent + login session megőrzése
- [ ] `webes_arak.html` Tanít gomb: valós Playwright recording (nem manuális lépés-építő)
- [ ] Attention layer: minden elfogadott ár/ajánlat/export → `golden_examples` visszacsatolás
- [ ] Excel export figyelő: módosítások detektálása → golden → LoRA
- [ ] Word export figyelő: módosítások detektálása → golden → LoRA

---

## 13. Böngésző mint szoftverkomponens

### 13.1 A három output-típus analógiája

A CRT három kimenet-típusa ugyanolyan súlyú szoftverkomponens:

| Komponens | Python csomag | CRT modul | Tanulási visszacsatolás |
|-----------|--------------|-----------|------------------------|
| Excel | `openpyxl` | `export_router.py` | módosított cellák → golden |
| Word | `python-docx` | `export_router.py` | módosított bekezdések → golden |
| **Böngésző** | **`playwright`** | **`web_sources.py`** | **elfogadott árak → golden** |

A Playwright **nem scraper-eszköz** — hanem a CRT böngésző-komponense, ugyanolyan rangon mint az openpyxl és python-docx.

### 13.2 Jelenlegi implementáció (v1.1)

```
web_sources    (tábla) — forrás URL-ek, login adatok titkosítva
web_scripts    (tábla) — tanított Playwright lépések JSON-ban, verziózva
web_prices     (tábla) — begyűjtött árak, cikktörzs-egyeztetéssel
web_sources.py (router) — CRUD + ping + script save/run API
webes_arak.html         — forráskezelő UI + Tanít gomb + árak megjelenítése
scrapers/govill_hu.py   — első működő scraper (urllib, 88 ár/futás, 0 hiba)
```

### 13.3 Tanítható scraper — működési elv

A „Tanít" gomb nem egyszerű felvevő — **verziókezelt workflow tárolás**:

```
1. Felhasználó megnyomja: Tanít – Navigálás
2. Manuálisan (v0.5) vagy Playwright recording-gal (v0.6+) rögzíti a lépéseket:
   navigate → https://szallito.hu/search?q=NYM
   wait_selector → .product-list
   extract → .product-name, .price
3. Script mentve: web_scripts (source_id, script_type='extract', version=N, steps=[...])
4. Forrás státusza: 'learned'
5. Ütemezett futtatás: POST /web/scripts/{id}/run → Playwright replay → web_prices
```

Előny a hagyományos hardkódolt scraperrel szemben:
- **Nincs kód** — a felhasználó tanítja, nem a fejlesztő írja
- **Login-képes** — B2B portálokra belép, session megmarad
- **GDPR-biztos** — valódi böngésző, consent egyszer klikkelve megmarad a profilban
- **Verziókezelt** — ha a webshop módosul, új verzió mentése, régi visszaállítható

### 13.4 Unified Attention Layer

Minden CRT output-típus ugyanazon visszacsatolási körön fut:

```
[Böngésző]   ──► elfogadott ár  ──┐
[Excel]      ──► fogadott export ──┼──► golden_examples ──► LoRA tanítás ──► AI javul
[Word]       ──► fogadott export ──┘
```

Implementáció:
- `web_prices.matched = true` → golden példa az ár-azonosításhoz
- Excel export: ha felhasználó módosít → delta rögzítve (`golden_router.py`)
- Word export: ha felhasználó módosít → delta rögzítve
- Minden golden → `lora_router.py` fine-tuning pipeline-ba kerül

Ez a CRT **önfejlesztő hurok** alapja: minél többet használják, annál pontosabb az AI.
...
| Playwright mint komponens | Nem scraper-tool, hanem a böngésző-réteg — openpyxl/python-docx analógiájára |
| Tanítható scraper | Felhasználó tanítja a workflow-t, nem fejlesztő írja — B2B/GDPR kompatibilis |
| Unified Attention Layer | Böngésző + Excel + Word visszacsatolás azonos golden pipeline-ba → LoRA self-improvement |
...
| Böngésző-komponens + Attention Layer | 2-3 hét | semmi (Playwright már telepítve) |
| **Összesen** | **~3,5 hónap** | **minimális** |
